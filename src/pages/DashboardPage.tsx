    ...
    <PageFrame>
      ...
      <div className="dashboard-grid">
        ...
        <Card className="dashboard-panel">
          ...
          <input className="text-input" value={whatIf} onChange={(event) => setWhatIf(event.target.value)} placeholder="Type a habit change" />
        ...
        <Card className="dashboard-panel">
          ...
          <ProgressBar value={(goals[0]?.savedAmount ?? 0) / Math.max(goals[0]?.targetAmount ?? 1, 1) * 100} tone="positive" />
          <MetricRow
            items={[
              { label: 'Saved', value: formatCompactINR(goals[0]?.savedAmount ?? 0) },
              { label: 'Target', value: formatCompactINR(goals[0]?.targetAmount ?? 0) },
              { label: 'Required / mo', value: formatCompactINR(
                goals[0]
                  ? (goals[0].targetAmount - goals[0].savedAmount) /
                      Math.max(1, new Date(goals[0].targetDate).getMonth() - new Date().getMonth() + 12 * (new Date(goals[0].targetDate).getFullYear() - new Date().getFullYear()))
                  : 0,
              ) },
            ]}
          />
