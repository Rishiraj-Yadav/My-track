...
export const exportCSV = (expenses: Expense[], goals: Goal[], profile: Profile) => {
  ...
  a.download = `architect-report-${date}.csv`
  a.click()
...
export const exportPDF = (
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.setTextColor(...gray)
    doc.text(
      `Generated by Architect · Page ${i} of ${totalPages}`,
      pageWidth / 2,
  ...
  doc.save(`architect-report-${new Date().toISOString().slice(0, 10)}.pdf`)
}
