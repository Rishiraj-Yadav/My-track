...
import { api, type SimulatorNlpParseResponse } from '../../lib/api'
import {
...
const averageConfidence = (ops: ParsedOp[]) =>
  ops.length === 0
    ? 0
    : ops.reduce((total, op) => total + (op.confidence ?? 0), 0) / ops.length
    ...
    <article className={`nlp-op-card nlp-op-card--${meta.tone}`}>
      <div className="nlp-op-card__header">
        <div className="nlp-op-card__chips">
          ...
          <span className="nlp-chip nlp-chip--target">{op.target ?? 'general'}</span>
          {amount !== 0 ? (
            <span className="nlp-chip nlp-chip--detail">
              {amount > 0 ? '+' : ''}
              {formatCompactINR(amount)}/mo
            </span>
          ) : null}
          {op.eventAmount ? (
        ...
        <span className="nlp-op-card__score">{Math.round(confidence * 100)}%</span>
      </div>
...
function HistoryList({
  ...
  return (
    <section className="nlp-history">
      ...
      {items.length === 0 ? (
        <div className="nlp-empty nlp-empty--history">
...
export function NLPConsole() {
  ...
  const [modelResult, setModelResult] = useState<SimulatorNlpParseResponse | null>(null)
  ...
  const fallbackResult = useMemo(
    () => (command.trim() ? parseMultiOp(command) : null),
    [command],
  )
  ...
  useEffect(() => {
    ...
    const requestId = window.setTimeout(async () => {
      try {
        const response = await api.parseSimulatorCommand(command)
        if (!cancelled) {
        ...
        if (!cancelled) {
          setModelResult(null)
          setParseError(error instanceof Error ? error.message : 'Model parse failed')
        }
      } finally {
        if (!cancelled) {
          setIsParsing(false)
  ...
  const result = useMemo<MultiOpResult | null>(() => {
    if (modelResult) {
      return {
        ops: modelResult.ops,
        primaryVerb: modelResult.primaryVerb ?? null,
        isAmbiguous: modelResult.isAmbiguous,
  ...
  return (
    <Card className="dashboard-panel dashboard-panel--wide">
      <div className="panel-head nlp-console__head">
        <div className="nlp-console__intro">
        ...
        <div className="nlp-status-row">
          ...
          <span className={`nlp-status ${modelResult?.provider === 'gemini' ? 'nlp-status--model' : ''}`}>
            {modelResult?.provider === 'gemini'
              ? modelResult.model
              : isParsing
                ? 'Parsing…'
                : 'Local fallback'}
      ...
      <div className="nlp-shell">
        <div className="nlp-main">
          <section className="nlp-composer">
            <div className="nlp-composer__label">
              <span className="nlp-section-label">Command builder</span>
              <p className="panel-copy">Write one change or combine multiple actions in one sentence.</p>
            </div>
            ...
            <div className="nlp-suggestions">
              <span className="nlp-suggestions__label">Quick prompts</span>
            ...
            {parseError ? (
              <div className="nlp-notice nlp-notice--warning">
                <span>{parseError}. Using fallback parser for now.</span>
              </div>
            ) : null}
          </section>
            ...
            <section className="nlp-preview">
              <div className="nlp-preview__header">
                ...
                <div className="nlp-preview__stats">
                  ...
                  {modelResult?.provider ? <span className="nlp-stat">{modelResult.provider}</span> : null}
              ...
              {result?.isAmbiguous && (result.alternatives?.length ?? 0) > 0 ? (
                <div className="nlp-notice nlp-notice--warning">
