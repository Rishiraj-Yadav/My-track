...
export function ScenariosPage() {
  ...
  const applyScenario = (scenarioId: string) => {
    ...
    if (!scenario) return
    void setSip({ monthlyAmount: scenario.monthlySip })
  }

  const applyTemplate = (name: string) => {
    setTemplate(name)
    if (!scenarios[0]) return
    if (name === 'stop-food') void setScenario(scenarios[0].id, { avoidableCut: 4200 })
    if (name === 'start-sip') void setScenario(scenarios[0].id, { monthlySip: sip.monthlyAmount + 3000 })
  }
    ...
    <PageFrame>
      ...
      <div className="dashboard-grid">
        ...
        <Card className="dashboard-panel dashboard-panel--wide">
          <div className="panel-head"><span>Outcome comparison</span><Pill tone="warning">Winner: {winner?.name ?? 'None yet'}</Pill></div>
        ...
        <Card className="dashboard-panel">
          ...
          {scenarios.length ? (
            ...
            <p className="panel-copy">Create a scenario in onboarding or from the backend-backed state to compare outcomes here.</p>
          )}
        </Card>
